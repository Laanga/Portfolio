...
const Experience: React.FC = () => {
  ...
  return (
    <section className="py-0 pr-4">
      ...
      >
        {/* Main heading - idéntico al About */}
        <h1 className="text-[48px] font-bold mb-8 leading-tight">Experience</h1>
        ...
        <div className="space-y-8">
          {experiences.map((exp, index) => (
            <div key={index} className="mb-8">
              {/* Job title */}
              <h2 className="text-[24px] font-bold mb-2 text-white">{exp.title}</h2>
              
              {/* Company and period */}
              <h3 className="text-[16px] font-medium mb-6 text-white/80">{exp.company}</h3>
              ...
              <div className="space-y-3">
                {exp.achievements.map((achievement, achIndex) => (
                  <div key={achIndex} className="flex items-start gap-3 text-white/80">
                    ...
                    <span className="text-[14px] leading-relaxed">{achievement}</span>
                  </div>
                ))}
              </div>
            </div>
